...
void loop() {
  ...
  if (debouncer.fell()) {  // If the button is pressed  
    ...
    if (firebase.setInt("Lamp 1 Field ID", 1)) {
      Serial.println("Lamp 1 set to 1 in Firebase.");
    } else {
      Serial.println("Failed to set Lamp 1 in Firebase.");
    }
  }
}

void checkStatus() {
  int led_status = firebase.getInt("Lamp 2 Field ID");
  ...
  if (led_status == 1) {
    ...
    firebase.setInt("Lamp 2", 0);  // Reset Lamp 2 in Firebase
  } else {
    Serial.println("OFF");
    fadeOutEffect(1000);  // Fade out the LEDs over 2 second
  }
}
